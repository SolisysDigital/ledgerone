...
export default async function HostingAccountPage({ 
  ...
  if (!response.ok) {
    console.error('Failed to fetch hosting account data:', response.status, response.statusText);
    return notFound();
  }
          ...
          <CardContent>
            <div className="bg-muted/10 rounded-lg p-4 border border-border/50" style={{ borderRadius: '0.5rem' }}>
              <Suspense fallback={<div>Loading entity relationships...</div>}>
                <DetailObjectRelationships detailObjectId={id} detailObjectType="hosting_accounts" />
              </Suspense>
            </div>
          </CardContent>
        </Card>
