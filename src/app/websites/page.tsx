...
export default function WebsitesPage() {
  ...
  return (
    <PageLayout
      title={config?.label || "Websites"}
      subtitle={`Manage your ${config?.label?.toLowerCase() || "websites"} data`}
      actionButton={
        <Button asChild className={STYLES.BUTTONS.PRIMARY}>
          <Link href={`/${tableName}/new`}>
            <div className={ICONS.PLUS} />
            Add New {config?.label?.slice(0, -1) || "Website"}
          </Link>
        </Button>
      }
    >
      <SearchSection
        placeholder={`Search ${config?.label?.toLowerCase() || "websites"}...`}
