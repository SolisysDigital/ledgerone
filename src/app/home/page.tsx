    ...
    <div className="min-h-screen">
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 relative">
                  ...
                  <Input 
                    ...
                    className="w-full h-16 pl-12 pr-24 text-lg rounded-xl shadow-lg bg-white/80 backdrop-blur-sm border-slate-200 focus:border-blue-500 transition-all duration-200"
                  />
                </div>
                <Button 
                  size="lg" 
                  className="h-16 px-8 rounded-xl shadow-lg bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 transition-all duration-200"
                >
      ...
      <div className="max-w-7xl mx-auto px-6 pb-16">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-600 text-lg">Navigate to your most important data</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(tableConfigs).slice(0, 6).map(([tableName, config]) => {
            ...
            return (
              <Card key={tableName} className="group hover:shadow-xl transition-all duration-300 border-slate-200 hover:border-blue-300 bg-white/90 backdrop-blur-md">
                <CardContent className="p-6">
                  <Link href={`/${tableName}`} className="block">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <ArrowRight className="h-5 w-5 text-slate-400 group-hover:text-blue-600 transition-colors duration-200" />
                    </div>
      ...
      {searchResults.length > 0 && (
        <div className="max-w-7xl mx-auto px-6 pb-16">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-slate-800 mb-4">Recent Data</h2>
            <p className="text-slate-600 text-lg">Your most recent entries across all tables</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {searchResults.slice(0, 6).map((result, index) => {
              ...
              return (
                <Card key={`${result.table}-${result.id}`} className="group hover:shadow-xl transition-all duration-300 border-slate-200 hover:border-blue-300 bg-white/90 backdrop-blur-md">
                  <CardContent className="p-6">
                    <Link href={`/${result.table}/${result.id}`} className="block">
                      ...
                      {result.type && (
                        <Badge variant="outline" className="mt-2 text-xs border-slate-200 text-slate-600">
                          {result.type}
                        </Badge>
                      )}
                    </Link>
