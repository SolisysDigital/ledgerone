...
export default function PhonesPage() {
  ...
  return (
    <PageLayout
      title={config?.label || "Phones"}
      subtitle={`Manage your ${config?.label?.toLowerCase() || "phones"} data`}
      actionButton={
        <Button asChild className={STYLES.BUTTONS.PRIMARY}>
          <Link href={`/${tableName}/new`}>
            <div className={ICONS.PLUS} />
            Add New {config?.label?.slice(0, -1) || "Phone"}
          </Link>
        </Button>
      }
    >
      <SearchSection
        placeholder={`Search ${config?.label?.toLowerCase() || "phones"}...`}
