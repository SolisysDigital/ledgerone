...
export default function BankAccountsPage() {
  ...
  return (
    <PageLayout
      title={config?.label || "Bank Accounts"}
      subtitle={`Manage your ${config?.label?.toLowerCase() || "bank accounts"} data`}
      actionButton={
        <Button asChild className={STYLES.BUTTONS.PRIMARY}>
          <Link href={`/${tableName}/new`}>
            <div className={ICONS.PLUS} />
            Add New {config?.label?.slice(0, -1) || "Bank Account"}
          </Link>
        </Button>
      }
    >
      <SearchSection
        placeholder={`Search ${config?.label?.toLowerCase() || "bank accounts"}...`}
